...
- ADDITION: новая информация, которая «прицепляется» к уже существующей фразе. При выборе ADDITION обязательно укажи ANCHOR — точную фразу в другом параграфе, к которой относится добавление. Anchor должен быть скопирован дословно из того параграфа, к которому он относится.
- «anchor» должен быть точной подстрокой одного из параграфов (без перефразирования). Предпочитай привязку к Параграфу 1, если добавление присутствует только во Втором параграфе, и к Параграфу 2, если добавление присутствует только в Первом.
