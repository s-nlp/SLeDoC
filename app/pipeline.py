...
def build_demo():
    with gr.Blocks(title="Semantic Mismatch · Pipeline", css=side_bar, theme=gr.themes.Soft()) as demo:
        gr.HTML(nav_tag)
        ...
        with gr.Tab("1. Extract claims"):
            ...
            model = gr.Textbox(label="OpenAI model (env configured)", value="gpt-4o-mini")
            ...
            def _run_extract(json_file, system_prompt, model_name):
                ...
                result_path = run_claim_extraction(input_path, system_prompt=system_prompt, model_name=model_name)
            ...
            run_btn.click(_run_extract, inputs=[in_pairs, sys_prompt, model], outputs=[download, out_path_box, preview])
